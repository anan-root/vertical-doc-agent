...
from construction_bidding_agent.document_parser.docx_table_index import (
    build_docx_table_index,
    write_table_index_outputs,
)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="构建 DOCX 表格索引。")
    parser.add_argument("--input", default=str(DEFAULT_INPUT), help=".docx 文件路径")
    parser.add_argument(
        "--json-output",
        default=str(ROOT / "outputs" / "json" / "docx_table_index.json"),
        help="JSON 输出路径",
    )
    parser.add_argument(
        "--report-output",
        default=str(ROOT / "outputs" / "reports" / "docx_table_index.md"),
    ...
    write_table_index_outputs(result, args.json_output, args.report_output)
